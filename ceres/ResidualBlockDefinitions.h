...
struct RegistrationError {
  ...
  bool operator()(const T* const poseEst1, const T* const poseEst2, T* residual) const { 
	...
	residual[_PSI_] = .001*relWeight*(x2Link[1] - x2Body1[1]);  // point of reference: .025rad = 1m error at 40m standoff distance
        residual[_B_] = .0001*relWeight*(AvgMotion + poseEst2[_B_]); //.001
        residual[_B_+1] = .0001*relWeight*(AvgMotion + poseEst1[_B_]); //10000.0*relWeight*((x2Link[1] - x2Body1[1])/(link.t2-link.t1) + poseEst2[_B_]);
	return true;
  }
   // Factory to hide the construction of the CostFunction object from
   // the client code.
