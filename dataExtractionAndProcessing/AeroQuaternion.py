...
def slerp(quat1, quat2, alpha):
   ...
   """
   difference = np.dot(quat1.q,quat2.q)
   SLERP_TO_LERP_SWITCH_THRESH = .01
   if( (1 - np.abs(difference)) > SLERP_TO_LERP_SWITCH_THRESH):
      theta = acos(np.abs(difference))
      oneOverSinTheta = 1/sin(theta)
      startWeight = sin(theta*(1-alpha))*oneOverSinTheta
      ...
      startWeight = 1-alpha
      endWeight = alpha
   resultX = quat1.q*startWeight + quat2.q*endWeight
   return Quaternion(normalize(resultX))
